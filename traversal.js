...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function calcSpot(spot) {
        ...
        while (iet < 80 && xtemp < r2) {
            ...
            if(typeof imgOffset == "number"){
                ...
                spot.complex.real = xtemp + realOffset  //+ spot.xmom*spot.zmom
            }else{
            ...
            if(spot == circ){
                ...
                if(lines.length < 1){
                    ////console.log(spot)
                    link = new Line(circ.x, circ.y, halfCanvasW+(spot.complex.real*halfCanvasW), halfCanvasH+(spot.complex.imaginary*halfCanvasH), "black", 2)
                }else{
                    link = new Line(lines[lines.length-1].x2, lines[lines.length-1].y2, halfCanvasW+(spot.complex.real*halfCanvasW), halfCanvasH+(spot.complex.imaginary*halfCanvasH), "black", 2)
                }
                lines.push(link)
            }
        }
        spot.g = ((Math.max(255 - (iet * 2.8), 0) + 1)*2)
        spot.r =  Math.max(((iet * redx)*3.5), 0)
        spot.b = 0 //Math.max((Math.sqrt((iet * bluex))*1), 0) 
    ...
    let circ = new Circle(350, 350, 4, "white")
